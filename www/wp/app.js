...
var app =
{
    ...
    fireAppEvent: function(evt, id){
        var e = evt.toLowerCase();
        if( e == "message"){
           var data = localStorageHelper.getItem(id);
           localStorageHelper.removeItem(id);
           message.fire(data);
        }else if(e == "start"){
            start.fire();
        }else if(e == "close"){
    ...
    sendMessage:function(data, appid){
        ...
        function generateUniqueMsgId()
        {
            ...
            while(null !== localStorageHelper.getItem(msgId))
            {
        ...
        if(args.length === 1){
            ...
            localStorageHelper.setItem(msgId, toString(data));
